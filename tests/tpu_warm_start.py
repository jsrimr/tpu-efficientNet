...
flags.DEFINE_integer("eval_steps", 1,
...
def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("stage1"):
        ...
        y = tf.layers.Conv2D(filters=32,
                             kernel_size=5,
                             padding="same",
                             activation="relu", name="myconv1")(image)
    ...
    # tf.train.init_from_checkpoint('gs://caplab-jeffrey/mnist_tpu_prac', {
    #     'model1/': 'model1'
    # })
...
def build_estimator(run_config):
    ckpt = tf.train.latest_checkpoint('gs://caplab-jeffrey/mnist_tpu_prac')
    # ckpt = tf.train.latest_checkpoint(FLAGS.model_dir)
    if ckpt:
        # var_name_and_shape_tuple_list = tf.train.list_variables(ckpt)
        # variables_list = [var_name for var_name, _ in var_name_and_shape_tuple_list if var_name.startswith("conv2d/")]
        ws = tf.estimator.WarmStartSettings(ckpt_to_initialize_from=ckpt, vars_to_warm_start=['stage1/', 'stage2/'])
        # ws = tf.estimator.WarmStartSettings(ckpt_to_initialize_from=ckpt, vars_to_warm_start=None)
    else:
        ws = None
